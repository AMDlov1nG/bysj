...
async def get_riot_auth_tokens(username, password):
    ...
    return entitlements_token, access_token, user_id

def getaccjson(entitlements_token, access_token, user_id):
    ...
    headers = {
        ...
        'X-Riot-ClientVersion':'release-06.03-shipping-6-830687'
    }
...
def get_skin_details(UID1,UID2,UID3,UID4):
    ...
    for i, levels_uuid in enumerate(levels_uuids):
        ...
        if matched_item:
            ...
            for j in range(chroma_num):
                ...
                display_icon = eval(f'uuid{i + 1}_cdisplayIcon{j + 1}')

                try:
                    matched_chroma = next(item for item in matched_item['chromas'] if
                                          item['displayIcon'] == display_icon)
    ...
    for i in range(1, 5):
        ...
        key = f'uuid{i}'
        obj[key] = {
            'Name': eval(f'uuid{i}_displayName'),
        }
        ...
        if eval(f'uuid{i}_displayName'):
            ...
            for j in range(1, 6):
                ...
                try:
                    ...
                    obj[key][f'cdisplayIcon{j}'] = eval(f'uuid{i}_cdisplayIcon{j}')
                except NameError:
                    print("None")
                    obj[key][f'cdisplayIcon{j}'] = None

                print(f"uuid{i}_ctreamedVideo{j}: ", end="")
                try:
                    print(eval(f'uuid{i}_ctreamedVideo{j}'))
                    obj[key][f'ctreamedVideo{j}'] = eval(f'uuid{i}_ctreamedVideo{j}')
                except NameError:
                    print("None")
                    obj[key][f'ctreamedVideo{j}'] = None
            ...
            for k in range(1, 6):
                ...
                try:
                    ...
                    obj[key][f'levelsVideo{k}'] = eval(f'uuid{i}_levelsVideo{k}')
                except NameError:
                    print("None")
                    obj[key][f'levelsVideo{k}'] = None

        else:

            print(f"uuid{i} not found")
        results.append(obj)
    with open('results23.json', 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=4)

    return results.append(obj)
...
def allapiget(username, password):
    username1 = username
    password1 = password
    ...
    uid3 = offers[2]
    uid4 = offers[3]
    get_skin_details(uid1, uid2, uid3, uid4)
    file_path= load_json_file (r'C:\Users\Mayn\Desktop\valorant-api\apifinal1.1\results23.json')
    return file_path
